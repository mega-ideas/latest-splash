...
function statusClass(status: SupportTicketStatus) {
  ...
  if (status === 'IN_REVIEW') return 'border-[var(--warn)] bg-[var(--warn-bg)] text-[#8b6418]';
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <div className="mb-2 inline-flex rounded-full bg-[#E39774]/10 px-3 py-1 text-xs font-bold uppercase tracking-wide text-[#9a4a2d]">Customer Operations</div>
          <h1 className="text-3xl font-black tracking-[-0.03em] text-[#1f4350]">Feedback and complaint management</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-[#326273]/90">Staff can triage incoming bugs, feature requests, complaints, and general support tickets separately from the customer dashboard.</p>
        </div>
        ...
        <div className="rounded-2xl border border-[#326273]/10 bg-white p-4 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button type="button" disabled={refreshing} onClick={() => void refreshTickets()} className="inline-flex items-center gap-2 rounded-lg border border-[#5C9EAD]/30 px-3 py-2 text-xs font-bold text-[#326273] hover:border-[#5C9EAD] disabled:opacity-60">
              {refreshing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Headphones className="h-4 w-4 text-[#237284]" />}
              Refresh
            </button>
          </div>
              ...
              <button key={ticket.id} type="button" onClick={() => setSelectedId(ticket.id)} className={`w-full rounded-2xl border p-4 text-left transition ${selected?.id === ticket.id ? 'border-[#5C9EAD] bg-[#5C9EAD]/10' : 'border-[#326273]/10 hover:border-[#5C9EAD]/40'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-[12px] font-bold uppercase tracking-wide text-[#326273]/90">{ticket.type} · {ticket.priority}</div>
                  </div>
                  <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[12px] font-bold ${statusClass(ticket.status)}`}>{statusLabels[ticket.status]}</span>
                </div>
                <p className="mt-3 line-clamp-2 text-xs leading-5 text-[#326273]/90">{ticket.message}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        {selected && (
          <div className="space-y-5">
            <section className="rounded-2xl border border-[#326273]/10 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 rounded-2xl bg-[#F6F0ED] p-5 text-sm leading-6 text-[#326273]/90">{selected.message}</div>
            ...
            <section className="rounded-2xl border border-[#326273]/10 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2 font-bold text-[#1f4350]"><MessageSquareReply className="h-5 w-5 text-[#237284]" /> Staff reply</div>
              <textarea value={replyMessage} onChange={(event) => setReplyMessage(event.target.value)} rows={6} placeholder="Write a customer reply or internal resolution note" className="w-full resize-none rounded-xl border border-[#326273]/70 bg-[#F6F0ED] px-4 py-3 text-sm outline-none focus:border-[#5C9EAD]" />
              <div className="mt-4 grid gap-2 sm:grid-cols-3">
                ...
                <button type="button" disabled={loading || !replyMessage.trim()} onClick={() => void updateTicket('REPLIED')} className="flex items-center justify-center gap-2 rounded-xl bg-[#237284] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#145d6a] disabled:opacity-60">
                  {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                  Send reply
                </button>
                <button type="button" disabled={loading} onClick={() => void updateTicket('CLOSED')} className="rounded-xl bg-[#E39774] px-4 py-3 text-sm font-bold text-[#073d49] transition hover:bg-[#E39774]/85 disabled:opacity-60">Close ticket</button>
              </div>
            </section>

            <section className="rounded-2xl border border-[#326273]/10 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2 font-bold text-[#1f4350]"><Mail className="h-5 w-5 text-[#237284]" /> Reply history</div>
              <div className="space-y-3">
                {selected.replies.length === 0 && <div className="rounded-xl bg-[#F6F0ED] p-4 text-sm text-[#326273]/90">No staff replies yet.</div>}
