...
export default function AuditPage({ params }: { params: Promise<{ intentId: string }> }) {
  ...
  if (!view) return <div className="mx-auto max-w-5xl py-20 text-center text-foreground">Loading audit chain...</div>;
  const rows = [
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><span className="dash-kicker">Private to the world, provable to the auditor</span><h1 className="dash-title mt-2">Audit trail</h1><p className="mt-1 font-mono text-[13px] text-foreground">{intentId}</p></div>
        <button onClick={verify} className="rounded-xl bg-accent px-5 py-3 font-bold text-[#073d49]">{verifyState === 'loading' ? 'Verifying...' : 'Verify document hash'}</button>
      </header>
      {verifyState !== 'idle' && verifyState !== 'loading' && <div className={`rounded-2xl p-4 font-bold ${verifyState === 'verified' ? 'bg-primary/15 text-[#1F4452]' : 'bg-destructive/10 text-[#1F4452]'}`}>{verifyState === 'verified' ? 'Hash verified' : 'Hash mismatch'}</div>}
      <section className="grid gap-3 md:grid-cols-2">
        {rows.map(({ icon: Icon, label, value }) => <div key={label} className="dash-block p-5"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.15em] text-foreground/90"><Icon className="h-4 w-4 text-foreground" /> {label}</div><div className="mt-3 break-all font-mono text-sm font-semibold">{value}</div></div>)}
      </section>
