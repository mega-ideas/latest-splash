  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-5">
      <header>
        ...
        <p className="mt-1 max-w-2xl text-[13px] font-medium text-[#326273]">
      ...
      <section className="dash-surface p-5 md:p-7">
        <div className="flex items-center gap-3">
          ...
          <div><h2 className="font-semibold text-[#326273]">Active rate holds</h2><p className="text-[13px] text-[#326273]/90">{active.length} ready to use</p></div>
        </div>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          {active.map((hold) => (
            <article key={hold.id} className="rounded-2xl border border-[#5C9EAD]/30 bg-[#5C9EAD]/10 p-5 shadow-[5px_6px_0_rgba(50,98,115,0.12)]">
              <div className="flex items-start justify-between gap-3">
                <div><span className="text-[12px] font-semibold uppercase tracking-[0.18em] text-[var(--info)]">Rate hold {hold.demo ? '· DEMO' : ''}</span><h3 className="mt-1 text-xl font-semibold text-[#326273]">USD → {hold.corridorCurrency}</h3></div>
          ...
          {active.length === 0 && <div className="col-span-full rounded-2xl border border-dashed border-[#326273]/20 p-8 text-center text-sm font-medium text-[#326273]/90">No active rate holds. Create one from the quote step of a transfer.</div>}
        </div>
      </section>

      {inactive.length > 0 && (
        <section className="dash-surface p-5 md:p-7">
          <div className="flex items-center gap-2 font-semibold text-[#326273]"><TimerReset className="h-4 w-4 text-[#9f5839]" /> Previous rate holds</div>
          <div className="mt-4 divide-y divide-[#326273]/10">
            {inactive.map((hold) => <div key={hold.id} className="flex items-center justify-between gap-4 py-3 text-sm"><span className="font-semibold text-[#326273]">USD → {hold.corridorCurrency}</span><span className="font-mono text-[#326273]/90">{hold.rate}</span><span className="rounded-full bg-[#326273]/8 px-2 py-1 text-[13px] font-bold text-[#326273]/90">{hold.state === 'ACTIVE' ? 'EXPIRED' : hold.state}</span></div>)}
          </div>
        </section>
      )}
    </div>
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-white/75 p-3"><div className="text-[12px] font-semibold uppercase tracking-wide text-[#326273]/90">{label}</div><div className="mt-1 font-mono font-semibold text-[#326273]">{value}</div></div>;
}
