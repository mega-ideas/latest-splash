...
| 3 | Fee wallet | Set on the founder's machine. **Production still needs it** (below). |
...
`SPLASH_FEE_ADDRESS_MAINNET` is Splash's wallet for the 0.80% fee on USDC wallet transfers. The founder created it in Slush: `0x0ca0…a5fe` (the full value is in the founder's `.env.local`).
...
- **To do:** set the same value as `SPLASH_FEE_ADDRESS_MAINNET` in the production host's environment. Until then, USDC transfers stay closed in production.
