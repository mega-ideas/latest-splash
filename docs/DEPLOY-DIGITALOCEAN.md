...
5. **Collect production configuration.** The environment is validated at
   ...
   **Decided by your flags**
   ...
   | Stablecoin lane | `SPLASH_FEE_ADDRESS_MAINNET` (no USDC transfer is quoted without it; `doctor` checks on mainnet that it is a wallet, not an object; the full value for production is in the founder's `.env.local`, see `docs/GO-LIVE-HANDOVER.md`), `CHAINALYSIS_SANCTIONS_API_KEY` to screen wallet recipients; optional `ETHEREUM_RPC_URL` / `USDY_ORACLE` and `DEEPBOOK_STABLE_PAIRS` (the old name `DEEPBOOK_STABLE_PAIR` is ignored). See `docs/STABLECOIN-LANE.md`, Configuration |
